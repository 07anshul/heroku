...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
  if (isset($_GET["ask"])) {
    if ($_GET["question"] != "") {
      $insert = "INSERT INTO user_questions (user_id, question) VALUES (?, ?)";

      // prepare insert statement
      if ($stmt = $mysqli->prepare($insert)) {
        $stmt->bind_param("is", $param_user_id, $param_question);
  ...
  if ($_SERVER["REQUEST_METHOD"] == "GET") {
    if (isset($_GET["add_comment"])) {
      ...
      if ($_GET["comment"] != "") {
        $insert = "INSERT INTO user_comments (question, user_id, comment) VALUES (?, ?, ?)";

        // prepare insert statement
        if ($stmt = $mysqli->prepare($insert)) {
          $stmt->bind_param("sis", $param_question, $param_user_id, $param_comment);
